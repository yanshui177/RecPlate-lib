function  [img_train, label_train, img_test, label_test, param_num]=...
    process(train, test, flag, param_num)
% 预处理训练测试数据集, 生成完整可用训练数据和测试数据 num
% --------------------------------------------------
% 参数：[img_train, label_train, img_test, label_test] = process(train, test, dim)
...
%% 标签向量化
one_hot=param_num.one_hot;
label_train=zeros(one_hot,1500); % 训练标签
for i=1:train_num
    for j=1:one_hot
...
label_test=zeros(one_hot,200); % 测试标签
for i=1:test_num
    for j=1:one_hot
...
param_num.cate=flag;
param_num.latent=latent;
param_num.img_size=size(train_images(:,:,1));
param_num.coef=coef;
